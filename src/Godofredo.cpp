...
Godofredo::Godofredo(int x, int y) {
	...
	sp = Sprite("img/LancelotIdleRight.png", 8, 120);
...
void Godofredo::UpdateCommands(float dt) {
	if(InputManager::GetInstance().IsKeyDown(SDLK_a)) {
		...
		facing = LEFT;
		if(hidden){
			if(sp.GetFile() != "img/LancelotRunningLeftInv.png") {
				sp.SetFile("img/LancelotRunningLeftInv.png", 8, 120);
				UpdateBoundingBox();
			}
		}else{
			if(sp.GetFile() != "img/LancelotRunningLeft.png") {
				sp.SetFile("img/LancelotRunningLeft.png", 8, 120);
				UpdateBoundingBox();
			}
		}
		...
		facing = RIGHT;
		if(hidden) {
			if(sp.GetFile() != "img/LancelotRunningRightInv.png") {
				sp.SetFile("img/LancelotRunningRightInv.png", 8, 120);
				UpdateBoundingBox();
			}
		}else{
			if(sp.GetFile() != "img/LancelotRunningRight.png") {
				sp.SetFile("img/LancelotRunningRight.png", 8, 120);
				UpdateBoundingBox();
			}
		}
	}else{
		...
		if(facing == LEFT) {
			if(hidden) {
				if(sp.GetFile() != "img/LancelotHiddenLeft.png") {
					sp.SetFile("img/LancelotHiddenLeft.png", 8, 120);
					UpdateBoundingBox();
				}
			}else{
				if(sp.GetFile() != "img/LancelotIdleLeft.png") {
					sp.SetFile("img/LancelotIdleLeft.png", 8, 120);
					UpdateBoundingBox();
				}
			}
		}else if(facing == RIGHT){
			if(hidden) {
				if(sp.GetFile() != "img/LancelotHiddenRight.png") {
					sp.SetFile("img/LancelotHiddenRight.png", 8, 120);
					UpdateBoundingBox();
				}
			}else{
				if(sp.GetFile() != "img/LancelotIdleRight.png") {
					sp.SetFile("img/LancelotIdleRight.png", 8, 120);
					UpdateBoundingBox();
				}
			}
		}
	...
	if(InputManager::GetInstance().IsKeyDown(SDLK_e)) {
		if(attackT.GetTime() < 0) {
			StageState::AddObject(new Attack("img/notattack.png", 2, box.x+(box.w/2), box.y, power, 200, facing));
			attackT.Start();
		}
	}
