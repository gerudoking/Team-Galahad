...
Notfredo::Notfredo(int x, int y) {
	sp = Sprite("img/NotfredoIdleRIght.png", 8, 120);
			...
			if(StageState::IsColliding(radius, player) && !Godofredo::player->IsHidden()) {
				...
				if(player.x < box.x) {
					if(attackCD.GetTime() < 0) {
						StageState::AddObject(new Notattack("img/notattack.png", 2, box.x+(box.w/2), box.y, power, 200, facing));
						attackCD.Start();
					}
					if(box.x - speed.x*dt < player.x)
						...
						box.x -= speed.x*dt;
					facing = LEFT;
					if(sp.GetFile() != "img/NotfredoRunningLeft.png") {
						sp.SetFile("img/NotfredoRunningLeft.png", 8, 120);
						UpdateBoundingBox();
					}
				}else{
					if(attackCD.GetTime() < 0) {
					...
					if(box.x + speed.x*dt > player.x)
						...
						box.x += speed.x*dt;
					facing = RIGHT;
					if(sp.GetFile() != "img/NotfredoRunningRight.png") {
						sp.SetFile("img/NotfredoRunningRight.png", 8, 120);
						UpdateBoundingBox();
					}
				}
				...
				if(looking.GetTime() < looking.GetLimit()) {
					...
					if(facing == LEFT) {
						facing = RIGHT;
						sp.SetFile("img/NotfredoIdleRight.png", 8, 120);
						UpdateBoundingBox();
					}else{
						facing = LEFT;
						sp.SetFile("img/NotfredoIdleLeft.png", 8, 120);
						UpdateBoundingBox();
					}
					idle.Start();
				}
